...
AsteroidGroup.prototype.create = function() {
  ...
  for(var i = 0; i < this.MAX_ASTEROIDS; i++) {
    ...
    var color = COLORS[randBetween(0, this.COLORS.length - 1)];
    var size = SIZES[randBetween(0, this.SIZES.length - 1)];
    var asteroid = new Asteroid(game, start, velocity, rotation, color, size);
    this.pool.add(asteroid.sprite);
  }
